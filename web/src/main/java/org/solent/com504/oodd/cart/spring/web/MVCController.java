...
public class MVCController {
    ...
    public String viewCart(@RequestParam(name = "action", required = false) String action,
            ...
            @RequestParam(name = "itemUUID", required = false) Long itemUuid,
        ...
        if (action == null) {
            ...
            ShoppingItem shoppingItem = shoppingService.getNewItemByID(id);
    ...
    public String catalogList(@RequestParam(name = "action", required = false) String action,
        ...
        if (action == null) {
            ...
            newShoppingItem.setQuantity(itemQuantity);
            //List< ShoppingItem> shoppingItemList = shoppingService.getNewItemById(null);
            if (newShoppingItem == null) {
                message = "cannot add unknown " + itemName + " to Catalog";
            } else {
                message = "adding " + itemName + " to Catalog price= " + itemPrice;
                shoppingService.addItemToCatalog(newShoppingItem);
            }
        ...
        } else if ("showEditItemFromCatalog".equals(action)) {

            message = "Editing " + itemName + " from Catalog";
            ShoppingItem item = shoppingService.getNewItemById(itemId);
            model.addAttribute("item", item);
            model.addAttribute("selectedPage", "admin");

            return "editCatalogItem";
        } else if ("editItemFromCatalog".equals(action)) {

            message = "Edited " + itemName + " from Catalog";
            shoppingService.updateCatalogItem(itemId,itemName,itemQuantity,itemPrice);
